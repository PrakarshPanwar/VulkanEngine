...
namespace VulkanCore {
	...
	{
	public:
		VulkanRenderCommandBuffer(const std::string& debugName, VkCommandPool cmdPool, CommandBufferLevel cmdBufLevel = CommandBufferLevel::Primary, uint32_t queryCount = 0);
		~VulkanRenderCommandBuffer();

		void Begin() const override;
		void Begin(VkRenderPass renderPass, VkFramebuffer framebuffer) const;
		void End() const override;
		void Execute(VkCommandBuffer secondaryCmdBuffers[], uint32_t count) const;
		...
		void RetrieveQueryPoolResults() const;
